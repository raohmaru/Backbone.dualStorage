 ...
 */
var S4, backboneSync, callbackTranslator, dualsync, getStoreName, getStoreType, localsync, memoryStorage, modelUpdatedWithResponse, onlineSync, parseRemoteResponse,
...
getStoreType = function(collection, model) {
  model || (model = collection.model.prototype);
  return _.result(collection, 'storeType') || _.result(model, 'storeType');
};

Backbone.Collection.prototype.syncDirty = function(options) {
  var i, id, ids, len, ref, results, store;
  store = localStorage.getItem((getStoreName(this)) + "_dirty") || memoryStorage.getItem((getStoreName(this)) + "_dirty");
...
Backbone.Collection.prototype.syncDestroyed = function(options) {
  ...
  store = localStorage.getItem((getStoreName(this)) + "_destroyed") || memoryStorage.getItem((getStoreName(this)) + "_destroyed");
  ids = (store && store.split(',')) || [];
  results = [];
  for (i = 0, len = ids.length; i < len; i++) {
    id = ids[i];
...
window.Store = (function() {
  ...
  function Store(name, type) {
    this.name = name;
    this.type = type;
    this.storage = this.getStorageInterface(type);
  ...
  Store.prototype.save = function() {
    return this.storage.setItem(this.name, this.records.join(','));
  };

  Store.prototype.recordsOn = function(key) {
    var store;
    store = this.storage.getItem(key);
  ...
  Store.prototype.dirty = function(model) {
    ...
    if (!_.include(dirtyRecords, model.id.toString())) {
      ...
      this.storage.setItem(this.name + '_dirty', dirtyRecords.join(','));
    }
    return model;
  };
  ...
  Store.prototype.clean = function(model, from) {
    ...
    if (_.include(dirtyRecords, model.id.toString())) {
      this.storage.setItem(store, _.without(dirtyRecords, model.id.toString()).join(','));
    }
    return model;
  };
  ...
  Store.prototype.destroyed = function(model) {
    ...
    if (!_.include(destroyedRecords, model.id.toString())) {
      ...
      this.storage.setItem(this.name + '_destroyed', destroyedRecords.join(','));
    }
    return model;
  };
  ...
  Store.prototype.create = function(model, options) {
    ...
    this.storage.setItem(this.name + this.sep + model.id, JSON.stringify(model.toJSON ? model.toJSON(options) : model));
  ...
  Store.prototype.clear = function() {
    ...
    for (i = 0, len = ref.length; i < len; i++) {
      id = ref[i];
      this.storage.removeItem(this.name + this.sep + id);
    }
    this.records = [];
    return this.save();
  };
  ...
  Store.prototype.hasDirtyOrDestroyed = function() {
    return !_.isEmpty(this.storage.getItem(this.name + '_dirty')) || !_.isEmpty(this.storage.getItem(this.name + '_destroyed'));
  };

  Store.prototype.find = function(model) {
    var modelAsJson;
    modelAsJson = this.storage.getItem(this.name + this.sep + model.id);
    if (modelAsJson === null) {
      return null;
    }
  ...
  Store.prototype.findAll = function() {
    ...
    for (i = 0, len = ref.length; i < len; i++) {
      id = ref[i];
      results.push(JSON.parse(this.storage.getItem(this.name + this.sep + id)));
    }
    return results;
  };

  Store.prototype.destroy = function(model) {
    this.storage.removeItem(this.name + this.sep + model.id);
  ...
  Store.prototype.getStorageInterface = function(type) {
    if (type === 'memory') {
...
window.Store.exists = function(storeName) {
  return localStorage.getItem(storeName) !== null || memoryStorage.getItem(storeName) !== null;
};
